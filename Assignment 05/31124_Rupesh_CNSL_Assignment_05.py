...
def lowerUpper():
    ...
    lower = '.'.join(str(int(i, 2))
                     for i in [lower[0: 8], lower[8:16], lower[16:24], lower[24:32]])
    upper = '.'.join(str(int(i, 2))
                     for i in [upper[0: 8], upper[8:16], upper[16:24], upper[24:32]])
    l = int(lower.split('.')[-1])
    u = int(upper.split('.')[-1])
    r = (l + u)//2
